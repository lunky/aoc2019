...
day7b' :: String -> String -> Int -> Int
day7b' programStr phaseStr seed = foldr (\y acc -> 
                                        snd $ intCode program [y,acc]) seed phase
  where program = parseInput programStr
        phase = map digitToInt phaseStr
...
--  | currInst == 99 = error ("found 99, output=" ++ show output ++ "curr=" ++ show curr ++ " - offset=" ++ show offset ++ " : " ++ show inst)
  | currInst == 99 = Left output
  | currInst == 3 && input==[] = Right (offset, inst, input, output) -- wait
...
intCode''' x = case (intCode'' x) of
  Left payload -> payload
  Right payload -> intCode''' payload
