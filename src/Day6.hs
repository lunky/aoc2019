...
day6b input = minimum $ map (\(a,b,c)->b-1+c-1)      
                    ...
                    $ catMaybes
                    $ map (\y -> findPlanet y planetTree)
                    $ reverse 
                    $ takeWhile (\y -> y/="YOU" && y/= "SAN") 
                    $ walkTree planetTree
...
parseInput input = map (\y -> (\[a,b]->(b,a)) $ splitOn ")" y) $ lines input
...
buildTree current planetMap = 
        (Planet current ( map (\y -> buildTree y planetMap) 
...
findPlanet search (Planet name planets)   
    ...
    | otherwise =  firstJust $ (\y -> findPlanet search y) <$> planets
